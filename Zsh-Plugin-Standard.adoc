...
5. Finally, the `:A` modifier will be applied in to resolve any
   symbolic links and unneeded `../` components, plus make the path absolute.
   This step is taken in order to provide the same trailing dir for different
   symbolic paths, because some plugins use it, e.g.: for location of plugin
   configuration in `~/.config/DIR` or similar.
